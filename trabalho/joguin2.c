...
int estado[] = {0}; //vetor com os estados dos jogadores: 0 = INATIVO // 1 = ATACANDO // 2 = DEFENDENDO
                   // 3 = DEFENDENDO DE 2
...
//entrada ataque
void IniciaAtaque (int id, int idDef) {
    pthread_mutex_lock(&mutex);
    printf("A[%d] quer atacar\n", id);
    while(totalRingue > P && estado[idDef] >= 3) {
        printf("A[%d] bloqueou\n", id);

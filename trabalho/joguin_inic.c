...
int main(void) {
    ...
    int ordem[P];
    int maior = 0; int maior_ind = 0; int j = 0; int limite_max = iniciativa_max + 1;
    for (int i = 0; i < P; i++) {
        maior = 0;
        maior_ind = 0;
        for (j = 0; j < P; j++) {
            if (lista_jogs[j].iniciativa > maior && lista_jogs[j].iniciativa < limite_max) {
                maior = lista_jogs[j].iniciativa;
                maior_ind = j;
            }
        }
        ordem[i] = maior_ind;

...
PRÓXIMOS PASSOS
    - alvos morrendo antes do atacante atacar pq outro estava atacando //acho q resolvi** mas acho q n precisa
    - jogador morto termina sua última ação // acho que eu resolvi
    - ajeitar o erro do jogador que ataca o que já está sendo atacado por 2 e que ataca quem tá morto //acho q resolvi**
    - implementar melhor a morte!!! // acho que eu resolvi
    - JOGADORES ESTÃO ATACANDO DEPOIS DE MORREREM // acho q resolvi**
    - jogadores estão defendendo depois do jogo terminar //acho q resolvi**
    - o programa realmente terminar quando declara um vencedor & tirar o deadlock (n sei pq ocorre)
PASSOS EVENTUAIS
...
int IniciaAtaque (int id_jogador, int id_thread) {
    ...
    while(1) {
        idAlvo = rand() % P;
        // so pode atacar se o jogador nao estiver sendo atacado por outros 2
        if((estado[idAlvo] <= 3) && (idAlvo != id_jogador) && (vivo[idAlvo] == 1) && lista_jogs[idAlvo].recebendo_ataque <=2) break;
    ...
    atacando++;
    lista_jogs[idAlvo].recebendo_ataque++;
    estado[id_jogador] = 1;
...
int main(void) {
    ...
    for(int i=0; i<P; i++){
        ...
        lista_jogs[i].recebendo_ataque=0;
        estado[i] = 0;
        vivo[i] = 1;

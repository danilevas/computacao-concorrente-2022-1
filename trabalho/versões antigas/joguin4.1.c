...
int IniciaAtaque (int id) {
    ...
    while(1) {
        idAlvo = rand() % P;
        if(estado[idAlvo] < 3) break;
        else printf("Jogador %d queria atacar jogador %d mas este já está sendo atacado por 2 jogadores\n", id, idAlvo);
    }
...
void * atacante (void * arg) {
    ...
    while(1) {
        if (vaiAtacar == 1) {
            int idAlvo = IniciaAtaque(id_jogador);

...
ESTADO ATUAL
    - vamo la ta bem bagunçado ainda kkkk mas pelo menos eu consegui:
        * fazer as threads só iniciarem depois de todas serem criadas (mas não ta funcionando direito)
...
PRÓXIMOS PASSOS
    - ajeitar essa questão do jogador 4 já começando a defender
    - entender porque algumas threads estão iniciando antes da main liberar elas
    ...
    - entender porque certos prints se repetem:
        * ex: "Thread defensora 17 do jogador 3 BLOQUEADA" (apareceu 2 vezes)
        * parece que esses prints nesse estilo estão todos se repetindo 2 vezes com o mesmo número e depois pulando o seguinte (1, 1, 3, 3, 5, 5...)
...
int main(void) {
    ...
    struct Passa passas[P];
    
    // cria as threads atacantes
    for(int j=0; j < P; j++) {
        for(int i=0; i < A; i++) {
            id[(j * A) + i] = (j * A) + i;
            passas[j].jogs = lista_jogs;
            passas[j].id_jogador = j;
            passas[j].id_thread = (j * A) + i;
            printf("Ciando thread atacante %d do jogador %d\n", (j * A) + i, j);
            if(pthread_create(&tid[(j * A) + i], NULL, atacante, (void *) &passas[j])) exit(-1);
            threads_ativas++;
    ...
    for(int j=0; j < P; j++) {
        for(int i=0; i < D; i++) {
            ...
            passas[j].jogs = lista_jogs;
            passas[j].id_jogador = j;
            passas[j].id_thread = (A * P) + (j * D) + i;
            printf("Ciando thread defensora %d do jogador %d\n", (A * P) + (j * D) + i, j);
            if(pthread_create(&tid[(A * P) + (j * D) + i], NULL, defensor, (void *) &passas[j])) exit(-1);
            threads_ativas++;
            //printf("Threads ativas = %d\n", threads_ativas);
    ...
    espera = 1; //podemos liberar as threads
